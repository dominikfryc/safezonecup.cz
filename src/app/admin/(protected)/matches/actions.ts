...
export async function generateSemifinals(formData: FormData) {
  ...
  const smallSfWinners = smallSfMatches.map((m) =>
    m.home_score > m.away_score ? m.home_team_id : m.away_team_id,
  );
  const smallSfLosers = smallSfMatches.map((m) =>
    m.home_score > m.away_score ? m.away_team_id : m.home_team_id,
  );
...
export async function generateGroupMatches(formData: FormData) {
  ...
  const availableGroups = Array.from({ length: tournament.number_of_groups }, (_, i) =>
    String(i + 1),
  );
  const numFields = tournament.number_of_fields || 1;
  const availableFields = Array.from({ length: numFields }, (_, i) => String.fromCharCode(65 + i));
  ...
  await supabase.from('matches').delete().eq('tournament_id', tournament_id).eq('stage', 'group');
