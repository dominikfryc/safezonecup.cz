...
const MatchNode = ({ match, title }: { match?: Match, title?: string }) => (
  <Card className="w-64 overflow-hidden flex flex-col relative z-10 shadow-lg border-primary/20">
    {title && <div className="text-[10px] font-bold uppercase tracking-wider bg-secondary px-3 py-1 text-muted-foreground">{title}</div>}
    {match ? (
...
export default function PlayoffTree({ tournamentId }: { tournamentId: string }) {
  ...
  return (
    <div className="flex flex-col gap-16 overflow-x-auto pb-8">
      
      {/* Big Playoff */}
      <div className="min-w-[800px]">
        <h3 className="text-2xl font-bold mb-8 flex items-center gap-3">
          <span className="w-3 h-8 bg-blue-500 rounded-full"></span>
          Big Playoff <span className="text-sm font-normal text-muted-foreground ml-2">Top 8 Teams</span>
        </h3>
      ...
      <div className="h-px bg-border w-full my-12"></div>

      {/* Small Playoff */}
      <div className="min-w-[600px]">
        <h3 className="text-2xl font-bold mb-8 flex items-center gap-3">
          <span className="w-3 h-8 bg-purple-500 rounded-full"></span>
          Small Playoff <span className="text-sm font-normal text-muted-foreground ml-2">Bottom 4 Teams</span>
        </h3>
        ...
        <div className="flex gap-16 relative">
          ...
          <div className="flex flex-col gap-8 justify-center">
            {[0, 1].map(i => (
              <div key={`ssf-${i}`} className="relative">
                <MatchNode match={smallSemis[i]} title={`Semifinal ${i + 1}`} />
                {/* Connecting lines right */}
                <div className="absolute top-1/2 -right-16 w-16 h-px bg-border"></div>
          ...
          <div className="flex flex-col justify-center relative left-16">
            <div className="relative">
              {/* Connecting lines left */}
              <div className="absolute top-1/2 -left-16 w-16 h-px bg-border"></div>
              <MatchNode match={smallFinal[0]} title="9th Place Match" />
            </div>
          </div>
        </div>
      </div>
