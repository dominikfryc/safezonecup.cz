    ...
    <div className="flex flex-col gap-6 mt-12">
      ...
      <div className="flex flex-col gap-3">
        {sortedSchedules.map((item) => (
          <Card
            key={item.id}
            className="overflow-hidden bg-card shadow-sm hover:shadow-md transition-all duration-300"
          >
            <CardContent className="p-4 flex items-center gap-6">
              ...
              <div className="text-lg font-medium text-foreground">{item.event}</div>
            </CardContent>
          </Card>
        ))}
