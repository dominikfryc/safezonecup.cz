  ...
  return (
    <div className="flex flex-col gap-12">
      ...
      {finishedMatches.length > 0 && (
        <div className="flex flex-col gap-6">
          <h3 className="text-xl font-bold text-muted-foreground">Poslední výsledky</h3>
...
function MatchCard({ match, state }: { match: Match; state: 'playing' | 'upcoming' | 'finished' }) {
  ...
  return (
    <Card
      className={`relative overflow-hidden transition-all duration-300 h-full ${isFinished ? 'bg-secondary/10' : 'shadow-xl'}`}
    >
      {state === 'playing' && (
      ...
      <CardContent className="p-6 h-full flex flex-col justify-center">
        ...
        <div className="flex justify-between items-center">
          <div className="text-right flex-1 min-w-0">
            <p
              className={`font-bold text-lg truncate ${isFinished && match.home_score < match.away_score ? 'text-muted-foreground' : 'text-foreground'}`}
            >
              {homeTeamName}
            </p>
          </div>
          ...
          <div className="text-left flex-1 min-w-0">
            <p
              className={`font-bold text-lg truncate ${isFinished && match.away_score < match.home_score ? 'text-muted-foreground' : 'text-foreground'}`}
            >
              {awayTeamName}
            </p>
          </div>
