...
export default function StandingsTable({ tournamentId }: { tournamentId: string }) {
  ...
  useEffect(() => {
    const fetchStandings = async () => {
      ...
      const stats = teams.reduce((acc: Record<string, TeamStats>, team: { id: string, name: string, group: string | null }) => {
        acc[team.id] = {
          id: team.id,
          name: team.name,
      ...
      matches?.forEach(m => {
        ...
        home.gs += m.home_score; away.gs += m.away_score;
        home.gc += m.away_score; away.gc += m.home_score;
        home.gd = home.gs - home.gc; away.gd = away.gs - away.gc;
    ...
    <div className="flex flex-col gap-8">
      <div className="grid lg:grid-cols-3 gap-6">
        {availableGroups.map(groupName => (
          <Card key={groupName} className="overflow-hidden shadow-xl border-t-4 border-t-primary/50">
            ...
            <Table>
              <TableHeader className="bg-secondary/10">
                <TableRow>
                  ...
                  <TableHead className="text-right text-primary font-bold">Pts</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {groups[groupName]?.map((team: TeamStats, index: number) => (
                  <TableRow key={team.id}>
                    ...
                    <TableCell className="text-center text-muted-foreground">{team.gd > 0 ? `+${team.gd}` : team.gd}</TableCell>
      ...
      <Card className="overflow-hidden shadow-xl bg-gradient-to-br from-background to-secondary/10 border-primary/20">
        <CardHeader className="flex flex-row items-center justify-between bg-secondary/20 pb-4">
          <CardTitle className="text-lg">Unified Standings (Playoff Qualification)</CardTitle>
          <p className="text-xs text-muted-foreground hidden sm:block">Top 8 advance to Big Playoff</p>
        </CardHeader>
        <Table>
          <TableHeader className="bg-secondary/10">
            <TableRow>
              ...
              <TableHead className="text-center hidden md:table-cell">GF:GA</TableHead>
              <TableHead className="text-center">GD</TableHead>
              <TableHead className="text-right text-primary font-bold">Pts</TableHead>
            </TableRow>
          </TableHeader>
              ...
              >
                <TableCell>
                  <div className="flex flex-col gap-0.5">
                    <span className={`font-bold ${index < 8 ? 'text-blue-500' : index < 12 ? 'text-purple-500' : 'text-muted-foreground'}`}>
                      #{index + 1}
                    </span>
                    <span className="text-[10px] text-muted-foreground uppercase leading-none">
                      {index < 8 ? 'Big' : index < 12 ? 'Small' : 'Out'}
                    </span>
                  </div>
                </TableCell>
                <TableCell className="font-bold">
                  {team.name} <span className="text-xs font-normal text-muted-foreground ml-2">(Grp {team.group})</span>
                </TableCell>
                <TableCell className="text-center text-muted-foreground">{team.played}</TableCell>
                <TableCell className="text-center text-muted-foreground hidden sm:table-cell">{team.w}</TableCell>
                <TableCell className="text-center text-muted-foreground hidden sm:table-cell">{team.d}</TableCell>
                <TableCell className="text-center text-muted-foreground hidden sm:table-cell">{team.l}</TableCell>
                <TableCell className="text-center text-muted-foreground hidden md:table-cell">{team.gs}:{team.gc}</TableCell>
                <TableCell className="text-center font-medium">{team.gd > 0 ? `+${team.gd}` : team.gd}</TableCell>
            ...
            {standings.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="h-32 text-center text-muted-foreground">
                  Standings will appear once teams are assigned.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
